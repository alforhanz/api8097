...
class SubTipo extends BaseController
{
...
//          'email' => 'required|min_length[6]|max_length[50]|valid_email|is_unique[client.email]',
    ...
    public function show($id,$tipo)
    {
        try {
            $model = new SeccionesModel();
            $subtipo = $model->findSubTipoById($id,$tipo);
            return $this->getResponse(
                [
                    'message' => 'Subtipo recuperado con exito',
                    'subtipo' => $subtipo
                ]
            );
        } catch (Exception $e) {
            return $this->getResponse(
                [
                    'message' => 'No se pudo encontrar el subtipo para la ID especificada'
                ],
