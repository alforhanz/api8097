...
//          'email' => 'required|min_length[6]|max_length[50]|valid_email|is_unique[client.email]',
    ...
    public function show($id)
    {
        try {
              ...
              if(!empty($_GET['ptipoclase'])) {$pTIPOCLASE = $_GET["ptipoclase"];}else {$pTIPOCLASE = null;}
              if(!empty($_GET['particulo'])) {$particulo = $_GET["particulo"];}else {$partparticuloculo = null;}
            $servicios = $model->BusquedaServicios($pTIPORPT,$pTIPOCLASE,$particulo);
              return $this->getResponse(
                  [
                      'msg'        => 'SUCCESS',
                      'message'    => 'Servicios recuperado con exito',
                      'servicios'    => $servicios,
                  ]
              );
        } catch (Exception $e) {
