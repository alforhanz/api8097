...
class VerificarNitCliente extends BaseController
{
...
//          'email' => 'required|min_length[6]|max_length[50]|valid_email|is_unique[client.email]',
    ...
    public function show($id)
    {
        try {
              $model = new Cliente_Model();
              if(!empty($_GET['nit'])) {$pNit = $_GET["nit"];}else {$pNit = null;}
              $nit = $model->sp_getClienteNit($pNit);
              return $this->getResponse(
                  [
                      'msg'        => 'SUCCESS',
                      'message'    => 'Nit recuperado con exito',
                      'nit'        => $nit,
                  ]
              );
            ...
            return $this->getResponse(
                [
                    'message' => 'No se pudo encontrar el Nit para la ID especificada'
                ],
